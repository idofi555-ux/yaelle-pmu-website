...
function showConfirmationModal(appointment) {
    ...
    const { formatDate, formatTime } = window.YaelleUtils || {};

    const formattedDate = formatDate ? formatDate(appointment.date) : appointment.date;
    const formattedTime = formatTime ? formatTime(appointment.time) : appointment.time;

    details.innerHTML = `
        <p><strong>Service:</strong> ${service?.name || appointment.service}</p>
...
function saveClient(appointment) {
    ...
    const existingIndex = clients.findIndex(c => c.email === appointment.email);
    ...
    if (existingIndex >= 0) {
        ...
        if (!client.appointments.includes(appointment.id)) {
